...
// [MIGRASI NAMED DATABASE]
// - Mengganti target Firestore ke "database-siyap"
...
export const onDisposisiSummaryUpdate = onDocumentWritten(
    {document: "disposisi/{disposisiId}", region: REGION, database: "database-siyap"},
    async (event) => {
...
export const onSuratSummaryUpdate = onDocumentUpdated(
    {document: "surat/{suratId}", region: REGION, database: "database-siyap"},
    async (event) => {
...
export const onTugasWritten = onDocumentWritten(
    { document: "tugas/{tugasId}", region: REGION, database: "database-siyap" },
    async (event) => {
...
export const onDrafPersetujuanWrite = onDocumentWritten(
    { document: "drafPersetujuan/{drafId}", region: REGION, database: "database-siyap" },
    async (event) => {
...
export const onNotificationCreated = onDocumentCreated(
    { document: "notifications/{notificationId}", region: REGION, database: "database-siyap" },
    async (event) => {
...
export const onJadwalTempatCreate = onDocumentCreated(
    { document: "jadwalTempat/{jadwalId}", region: REGION, database: "database-siyap" },
    async (event) => {
        const snap = event.data;
        if (!snap) {
...
export const onUserCreate = onDocumentCreated(
    { document: "users/{userId}", region: REGION, database: "database-siyap" }, // userId disini adalah NIP
    async (event) => {
        const snap = event.data;
        if (!snap) {
...
export const onJabatanWriteForDenormalization = onDocumentWritten(
    { document: "jabatan/{jabatanId}", region: REGION, database: "database-siyap" },
    async (event) => {
...
export const onPengumumanDelete = onDocumentDeleted(
    { document: "pengumuman/{pengumumanId}", region: REGION, database: "database-siyap" },
    async (event) => {
        const snap = event.data;
        if (!snap) return;
...
export { extractSuratDataAIV2 } from "./aiFunctions";
